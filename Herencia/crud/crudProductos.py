...
class CrudProductos ( icrud.ICrud ) :
    ...
    def buscar_informacion(self, id_producto):
        for producto in self.productos:
            if producto.idProducto == id_producto:
                return [producto.nombreProducto, producto.precio]
    ...
    def cambiar(self, listaProductos):
        ...
        for pedido in listaProductos:
            for producto in self.productos:
                if producto.nombreProducto == pedido:
                    productos_obtenidos.append(producto)
                    costo = costo + int(producto.precio)
        return {'productos' : productos_obtenidos , 'costo': costo}
    
    def obtener_id(self):
        productos = []
        for producto in self.productos:
            productos.append(producto.idProducto)
        return productos
    
    def obtener_id_individual(self, producto):
